...
function getCdnUrls(localPath) {
  return {
    cdn_primary: `https://cdn.jsdelivr.net/npm/@identitate-md/logos@${CDN_VERSION}${localPath}`,
    cdn_fallback: `https://unpkg.com/@identitate-md/logos@${CDN_VERSION}${localPath}`,
    local: localPath,
  };
}
 ...
 */
function migrateAll() {
  const files = readdirSync(INSTITUTIONS_DIR).filter((f) =>
    f.endsWith(".json"),
  );
  ...
  if (migratedCount > 0) {
    ...
    console.log(
      `🔗 CDN URLs folosesc: jsDelivr (primary), unpkg (fallback), local (backup)`,
    );
  } else {
