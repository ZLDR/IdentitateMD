...
async function callOpenRouter(apiKey, systemPrompt, userPrompt, attempt = 1) {
  ...
  try {
    ...
    throw new Error(
      "OpenRouter returned non-JSON response (first 200 chars): " +
        rawBody.slice(0, 200),
    );
  }
  ...
  if (!content)
    throw new Error(
      "Empty response from OpenRouter. Full response: " +
...
function extractTypographyFromMetadata(rawContent) {
  ...
  const familyOnlyPattern =
    /font\s*:?\s+([a-zA-Z][a-zA-Z0-9 +\-]{1,40})(?:\s*$|\s*\n)/im;
...
function extractWebsiteFromMetadata(rawContent) {
  ...
  for (const line of lines) {
    ...
    if (urlMatch) {
      ...
      if (
        /fonts?\.google|myfonts\.com|fontlibrary|fontsquirrel|adobe.*fonts/i.test(
          url,
        )
      )
        continue;
...
function discoverAssets(slugDir) {
  ...
  if (totalFiles === 0) {
    const rootFiles = fs
      ...
      .filter(
        (f) =>
          (f.endsWith(".svg") || f.endsWith(".png")) && f !== "favicon.svg",
      );
...
function buildAssetsFromFiles(slug, discovered, isFlat = false) {
  ...
  else if (assets.vertical)
    assets.main = { ...assets.vertical, type: "vertical" };
...
function buildUserPrompt(slug, discovered, metadata) {
  ...
  if (!Object.keys(frontmatter).length && !body.trim()) {
    prompt +=
      "No metadata.md found. Infer information from the slug and filenames only.\n";
  }
...
function normalizeAiResponse(data, slug) {
  ...
  if (d.typography && typeof d.typography === "object") {
    ...
    if (d.typography.primary && d.typography.primary.weights) {
      d.typography.primary.weights = normalizeWeights(
        d.typography.primary.weights,
      );
    }
    if (d.typography.secondary && d.typography.secondary.weights) {
      d.typography.secondary.weights = normalizeWeights(
        d.typography.secondary.weights,
      );
    }
  }
  ...
  if (!d.typography && (d.font || d.fonts)) {
    ...
    if (typeof fontVal === "string") {
      d.typography = {
        primary: { family: fontVal, url: null, weights: [] },
        secondary: null,
      };
    }
    delete d.font;
...
async function processSlug(slug, apiKey) {
  ...
  if (totalFiles === 0) {
    warn(
      slug +
        ": no SVG/PNG files found in layout subdirs or flat naming, skipping.",
    );
    return null;
  }
  ...
  if (fs.existsSync(mdPath)) {
    ...
    if (extractedUrl) {
      log(
        slug + ': extracted website ("' + extractedUrl + '") from metadata.md',
      );
  ...
  if (!normalized.colors || normalized.colors.length === 0) {
    ...
    if (svgColors) {
      log(
        slug + ": extracted " + svgColors.length + " color(s) from SVG files",
      );
